...
static CGFloat const kBackgroundMaskAlpha = 0.3;
...
- (void)generateView {
    ...
    // otherwise send the video view to the back if we have one
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:AVPlayerItemDidPlayToEndTimeNotification object:[self.player currentItem] queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *notification)
                                  {
...
- (void)pageViewController:(UIPageViewController *)pageViewController didFinishAnimating:(BOOL)__unused finished previousViewControllers:(NSArray *)__unused previousViewControllers transitionCompleted:(BOOL)completed {
...
- (void)blurBackground {
    ...
    UIColor *tintColor = [UIColor colorWithWhite:0.7f alpha:0.3f];
