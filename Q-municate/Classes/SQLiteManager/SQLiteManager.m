...
- (NSInteger)insertQuery:(NSString *)sql {
    NSInteger insertedRow = -1;
    ...
    if (openError == nil) {
        ...
        insertedRow = [self getLastInsertRowID];
...
- (NSArray *)getRowsForQuery:(NSString *)sql {
	...
	if (returnCode == SQLITE_ERROR) {
		const char *errorMsg = sqlite3_errmsg(db);
		NSError __unused *errorQuery = [self createDBErrorWithDescription:[NSString stringWithCString:errorMsg encoding:NSUTF8StringEncoding]
                                                         andCode:kDBErrorQuery];
		//NSLog(@"%@", errorQuery);
	}
...
- (NSString *)getDatabaseDump {
	...
	for (NSUInteger i = 0; i<[rows count]; i++) {
		...
		for (NSUInteger j = 0; j<[tableContent count]; j++) {
			...
			id _obj;
			while (_obj = [enumerator nextObject]) {
				[dump appendString:[NSString stringWithFormat:@"%@,",_obj]];
			}
			...
			while (_obj = [enumerator nextObject]) {
				...
				if ([_obj isKindOfClass:[NSNumber class]]){
					[dump appendString:[NSString stringWithFormat:@"%@,",[_obj stringValue]]];
				}
				//if it's a null
				else if ([_obj isKindOfClass:[NSNull class]]){
